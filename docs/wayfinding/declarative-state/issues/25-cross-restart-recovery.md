...
4. **持久身份只有"应用 + pid + bundle"。** `SavedWindow { window_id, pid, bundle_id }`；`window_id` 是 WindowServer 的进程内编号，现有文档明确"数值 ID 是候选提示，不是跨 daemon 身份"。
...
1. **身份连续性证据的层级**：什么算"同一个窗口"跨 daemon？倾向：应用 bundle + PID 存活 + AX 可读属性（role/subrole/title 精确匹配，或几何匹配）的**组合证据**，且证据不足时不绑定；`window_id` 永不作为身份。
