...
18:05:11的新录屏和用户复测进一步排除了tab作为必要条件：两个普通
Finder窗口互相切换也有激活状态闪动。此前只保留native身份切换缓存的
修正不足以解决这一通用问题。自动tiled stacking原先每次确认焦点改变
都对整条strip执行AXRaise，包括不重叠、无需重排的前一个Finder窗口。

本轮将自动请求限定为显示器范围内确实相交的观测矩形，保持原有远到近
顺序；仅在确实抬升背景重叠窗口时最后抬升焦点窗口。互不重叠时不执行
AXRaise。显式抬升整层仍保留完整顺序。已知几何动作未完成时推迟自动
修正；滚动前后观测到不同的真实遮挡关系仍可分别修正，不保证全程只有
一次请求。新增无tab双窗口连续往返的零raise回归，并保留边缘遮挡、
Mission Control、其他Space与显式抬升覆盖。尚未用最终构建实机复测，
重叠窗口必须调用AXRaise时是否仍有应用级激活副作用属于剩余风险。

随后用户提供17:58:23的2.68秒录屏：两个独立Finder窗口中，一个带tab，
另一个不带tab；外框基本稳定，切换tab时标题栏和红绿灯激活状态短暂变化。
这与持续尺寸抖动是不同的可见现象。回归测试确认，native root更换期间的
短暂Untracked焦点会清空tiled stacking缓存，重新确认同一布局entity后
仍raise整个strip，其中包括另一个独立Finder窗口。`raise_without_focus`
底层实际调用AXRaise，不能仅凭方法名假定应用不会改变主窗口状态。

修正仅在未识别焦点的PID及window ID与原布局窗口保留的native owner相符
时保留层级请求缓存；不提前宣称焦点已确认，也不对其他独立窗口的切换
