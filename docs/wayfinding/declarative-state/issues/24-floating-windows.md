...
> **2026-09-16 反转**：本票的"保存与候选隔离"半被 [ADR 0010](../../../adr/0010-runtime-state-rebuilt-from-rules.md) 取代——浮动帧不再落盘、不再作为候选导入；重启时浮动帧按"规则（`grid`）→ 观测（窗口自身位置）→ 视口内默认"重建。运行时行为（帧权威、修复）不变。落地见 [运行期状态与实现对账](../../runtime-state/map.md)。
