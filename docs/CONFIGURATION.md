...
| `automatic_reconcile` | Boolean | `true` | Re-syncs tracked window state on a one-second heartbeat and after every mouse-up. Both sweeps read every tracked application's accessibility window list, which is the largest periodic cost on the main thread. When disabled, only notifications that name what changed are reconciled, and `spool action reconcile-windows` still asks for a full one. |
