...
`initialization_postpones_the_alignment_and_judges_it_afterwards`：用守护进程自己的启动门保持启动窗口打开（一个仍带 `ExistingMarker` 的实体让 `finish_setup` 无法完成，于是 `Initializing` 留着），期间做一次做不到的宽度编辑 → **不判定**（尝试数为 0、回合不 blocked、零对齐记录、意图保留）；撤掉启动门后 → 正常判定并对齐到显示（一条记录）。

**但必须如实记下**：把 `postponed` 里的 `initializing` 项去掉，这条用例**依然通过**。原因是启动期本来就无法判定——窗口进 strip 与 `Initializing` 被移除发生在同一次 `finish_setup` 里（移除是延迟命令，在本帧 reconcile 之前生效），所以"启动期的实现回合"在真实进程里几乎不存在。也就是说 `initializing` 这项今天**是防御性冗余**，保留它是为了忠实于用户"启动期同规则"的裁决，而不是因为它在起作用。哪天真出现"窗口在 `Initializing` 期间就有活跃回合"的场景，它才生效，届时需要有能区分它的用例。
